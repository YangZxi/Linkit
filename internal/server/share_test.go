...
func TestDownloadHandler_本地文件启用协商缓存_命中ETag返回304(t *testing.T) {
	...
	code := "ABC123"
	if _, err := store.Client.ExecContext(ctx, `INSERT INTO share_code(resource_id, code, user_id, created_at) VALUES(?,?,?, CURRENT_TIMESTAMP)`, resourceID, code, 1); err != nil {
		t.Fatalf("插入短链失败: %v", err)
	}

	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.GET("/r/:slug", DownloadHandler(store, reg))
	...
	var viewCount int64
	if err := store.Client.QueryRowContext(ctx, `SELECT view_count FROM share_code WHERE code = ?`, code).Scan(&viewCount); err != nil {
